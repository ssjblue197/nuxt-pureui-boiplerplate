<template>
  ...
  >
      ...
      :class="
        twMerge(
          ...
          props.data?.router?.name === route.name
            ...
            : '',
          !systemStore.isCollapseSidebar ? 'px-lg' : 'justify-center',
        )
      "
    >
      <p-dropdown v-if="systemStore.isCollapseSidebar" hoist>
        <span slot="trigger">
          <p-tooltip :content="props.data?.name" hoist placement="right">
            <p-icon-button
    ...
    <div
      ...
      "
    >
      <SidebarItem v-for="i in props.data?.children" :key="i.id" :data="i" />
    </div>
  </div>
</template>
